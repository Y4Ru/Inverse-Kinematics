...
public class GraspableObject : MonoBehaviour
{
    ...
    void Start()
    {
        //ResetPositionAndOrientation(0, transform.position);
    }
    ...
    public virtual void OnGrasp(GameObject hand)
    {
        ...
        if (this.BreakableJoint != null)
        {
            ...
            if (breakJoint != null)
            {
                ...
                // 27.06.2017: we do not destroy the joint, we just remove the reference
    ...
    public virtual bool OnRelease(GameObject hand)
    {
        ...
        /* 27.06.2017: we do not enable the joint again, if the object is released underway it should just fall down 
        if (this.BreakableJoint != null) {
          Joint breakJoint = this.BreakableJoint.GetComponent<Joint>();
          if (breakJoint != null) {
    ...
    void OnCollisionEnter(Collision other)
    {
        ...
        if (other.gameObject.transform.root.gameObject.GetComponent<RigidHand>() != null)
        {
            ...
            /*
            if (GraspableObject.Verbose) Debug.Log ("Hit the hand: " + other.relativeVelocity);
            this.GetComponent<Rigidbody>().velocity += other.relativeVelocity * 2.0f;
            StartCoroutine(this.CollisionDispatcher(other.transform.root.gameObject));
            */
        }
        else
        {
